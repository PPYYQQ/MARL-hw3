...
| HAPPO | `8m_vs_9m` | HARL tuned config | smoke test 与 10000-step pilot 通过，full latest synced checkpoint 为 5120000 steps |
...
- 当前已同步 `MAPPO` + `3s5z` 的 250 个 full evaluation、`HAPPO` + `3s5z` 的 250 个 full evaluation、`MAPPO` + `8m_vs_9m` 的 250 个 full evaluation，以及 `HAPPO` + `8m_vs_9m` 的 64 个 early full evaluation 到 `results/raw/full/`。
...
- `results/processed/progress_summary.csv` 当前共有 886 行，其中 full 贡献 814 行。
...
- `8m_vs_9m`：smoke/pilot/full 曲线已生成，`MAPPO` 已完成 20000000 steps，`HAPPO` full latest synced checkpoint 为 5120000 steps。
