...
| HAPPO | `8m_vs_9m` | HARL tuned config | smoke test 与 10000-step pilot 通过，原始 full latest synced checkpoint 为 5120000 steps；recovery run 已同步到 3280000 steps |
...
- 当前已同步 `MAPPO` + `3s5z` 的 250 个 full evaluation、`HAPPO` + `3s5z` 的 250 个 full evaluation、`MAPPO` + `8m_vs_9m` 的 250 个 full evaluation、`HAPPO` + `8m_vs_9m` 原始 run 的 64 个 early full evaluation，以及 `HAPPO` + `8m_vs_9m` recovery run 的 41 个 early full evaluation 到 `results/raw/full/`。
...
- `HAPPO` + `8m_vs_9m` recovery run 已同步到 3280000 environment steps，final eval reward 为 17.7460，final eval win rate 为 0.725；该 run 仍在继续。
- `results/processed/progress_summary.csv` 当前共有 927 行，其中 full 贡献 855 行。
...
| 2026-05-31 | `SESSION=hw3_recover_happo_8m SEEDS=1 MAPS=8m_vs_9m ALGOS=happo EXP_PREFIX=hw3_recover bash scripts/launch_training_tmux.sh full` | 1 | `external/HARL/examples/results/smac/8m_vs_9m/happo/hw3_recover_full_happo_8m_vs_9m/seed-00001-2026-05-31-20-32-33` | `HAPPO` + `8m_vs_9m` recovery run 已同步到 3280000 steps |

## 观察模板

- `3s5z`：smoke/pilot/full 曲线已生成，`MAPPO` 与 `HAPPO` 都已完成 20000000 steps。
- `8m_vs_9m`：smoke/pilot/full 曲线已生成，`MAPPO` 已完成 20000000 steps，`HAPPO` 原始 full latest synced checkpoint 为 5120000 steps，recovery run 已同步到 3280000 steps。
