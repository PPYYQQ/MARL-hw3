...
| MAPPO | `3s5z` | 基于同地图 HAPPO tuned config 生成 MAPPO 配置 | smoke test 与 10000-step pilot 通过，full 已启动并同步到 960000-step checkpoint |
...
- 当前已同步 `MAPPO` + `3s5z` 的 12 个 full evaluation 到 `results/raw/full/`。
- `results/processed/progress_summary.csv` 当前共有 84 行，其中 full 贡献 12 行。
- 当前最新 full checkpoint 位于 960000 environment steps，eval reward 为 18.0869，eval win rate 为 0.6341；当前最佳 win rate 为 0.65。
...
| 2026-05-31 | `SESSION=hw3_full_20260531_seed1 SEEDS=1 EXP_PREFIX=hw3_full bash scripts/launch_training_tmux.sh full` | 1 | `external/HARL/examples/results/smac/3s5z/mappo/hw3_full_full_mappo_3s5z/seed-00001-2026-05-31-00-04-56` | 正式训练运行中；已同步到 960000-step checkpoint |
