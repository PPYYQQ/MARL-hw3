...
| MAPPO | `8m_vs_9m` | 基于同地图 HAPPO tuned config 生成 MAPPO 配置 | smoke test 与 10000-step pilot 通过，full latest synced checkpoint 为 1840000 steps |
...
- 当前已同步 `MAPPO` + `3s5z` 的 250 个 full evaluation、`HAPPO` + `3s5z` 的 250 个 full evaluation，以及 `MAPPO` + `8m_vs_9m` 的 23 个 early full evaluation 到 `results/raw/full/`。
- `MAPPO` + `8m_vs_9m` latest synced checkpoint 为 1840000 environment steps，final eval reward 为 12.3626，final eval win rate 为 0.2；当前最佳 win rate 为 0.2。
- `results/processed/progress_summary.csv` 当前共有 595 行，其中 full 贡献 523 行。
...
| 2026-05-31 | `SESSION=hw3_full_20260531_seed1 SEEDS=1 EXP_PREFIX=hw3_full bash scripts/launch_training_tmux.sh full` | 1 | `external/HARL/examples/results/smac/8m_vs_9m/mappo/hw3_full_full_mappo_8m_vs_9m/seed-00001-2026-05-31-10-59-45` | `MAPPO` + `8m_vs_9m` latest synced checkpoint 为 1840000 steps |

## 观察模板

- `3s5z`：smoke/pilot/full 曲线已生成，`MAPPO` 与 `HAPPO` 都已完成 20000000 steps。
- `8m_vs_9m`：smoke/pilot/full early checkpoint 曲线已生成，`MAPPO` full latest synced checkpoint 为 1840000 steps。
