...
- `results/raw/full/`：正式训练同步进来的 `progress.txt`；当前包含 `MAPPO` + `3s5z` 完整 20000000-step 结果，以及 `HAPPO` + `3s5z` 到 19040000-step 的阶段性结果。
...
- 正式训练：当前完成 1000-step smoke test 和 4 组 10000-step pilot；单 seed full 训练已在 `hw3_full_20260531_seed1` 中运行，`MAPPO` + `3s5z` 已同步完整 20000000-step 结果，`HAPPO` + `3s5z` 已同步到 19040000-step checkpoint。
