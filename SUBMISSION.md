...
- `results/raw/full/`：正式训练同步进来的 `progress.txt`；当前包含 `MAPPO` + `3s5z` 与 `HAPPO` + `3s5z` 的完整 20000000-step 结果，以及 `MAPPO` + `8m_vs_9m` 的 latest synced 480000-step 早期 checkpoint。
...
- 正式训练：当前完成 1000-step smoke test 和 4 组 10000-step pilot；单 seed full 训练已在 `hw3_full_20260531_seed1` 中运行，`MAPPO` + `3s5z` 与 `HAPPO` + `3s5z` 均已同步完整 20000000-step 结果，`MAPPO` + `8m_vs_9m` latest synced checkpoint 为 480000 steps。
